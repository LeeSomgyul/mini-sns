...
import type{ PostDto, FeedResponse } from "../../feed/types/feedResponseType";
...
export const usePostLikeMutation = () => {
    ...
    return useMutation({
        ...
        onMutate: async({postId, isCurrentlyLiked}: UsePostLikeProps) => {
            ...
            await queryClient.cancelQueries({queryKey: FEED_KEYS.detail(postId)});

            // 기존 캐시 데이터 가져오기
            const previousFeed = queryClient.getQueryData<FeedResponse>(FEED_KEYS.lists());
            const previousFeedDetail = queryClient.getQueryData<PostDto>(FEED_KEYS.detail(postId));
            ...
            if(previousFeedDetail){
                queryClient.setQueryData<PostDto>(FEED_KEYS.detail(postId), (oldDetail) => {
                    if(!oldDetail) return oldDetail;
                    return{
                        ...oldDetail,
                        // 현재 좋아요 상태 반전
                        isLiked: !isCurrentlyLiked,
                        // 좋아요가 눌러져 있었다면 -1, 안 눌러져 있었다면 +1
                        likeCount: isCurrentlyLiked 
                            ? oldDetail.likeCount - 1 
                            : oldDetail.likeCount + 1
                    };
                });
            }

            return {previousFeed, previousFeedDetail};
        },
        ...
        onError: (_err, variables, context) => {
            ...
            if(context?.previousFeedDetail){
                queryClient.setQueryData(FEED_KEYS.detail(variables.postId), context.previousFeedDetail);
            }
        },
    });
};

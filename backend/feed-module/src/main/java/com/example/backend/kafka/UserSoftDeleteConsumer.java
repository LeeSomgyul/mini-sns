...
import com.example.backend.service.FeedUserCacheService;
...
public class UserSoftDeleteConsumer {
    ...
    private final FeedUserCacheService feedUserCacheService;

    @KafkaListener(
            topics = KafkaTopics.USER_SOFT_DELETED_TOPIC,
            groupId = KafkaGroupId.GROUP_FEED_USER_SOFT_DELETE,
    ...
    public void consume(UserSoftDeleteEvent event){
        ...
        try{
            feedUserCacheService.softDeleteFeedUserCache(event.userId());

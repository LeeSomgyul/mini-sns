        ...
        try{
            ...
            if(!followerIds.isEmpty()){
                List<String> followerCacheKeys = followerIds.stream()
                        .map(followerId -> String.format(REDIS_USERSEARCH_FOLLOW_CACHE_PREFIX, followerId))
                        .toList();
            ...
            // 3. 탈퇴자 본인 레디스 캐시 삭제
            String myCacheKey = String.format(REDIS_USERSEARCH_FOLLOW_CACHE_PREFIX, targetUserId);
